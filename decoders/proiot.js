...
// To use with ProIoT
function DecoderProIoT(bytes, fPort) {
    // flat output (like original decoder):
    var response = {};
    lppDecode(bytes, 1).forEach(function (field) {
        ...
        if (typeof field['value'] === 'object') {
            ...
            for (const prop in values) {
                var name = field['name'] + '_' + field['channel'] + '_' + prop;
                response[name] =
                {
                    name: name,
                    value: values[prop]
            ...
            var name = field['name'] + '_' + field['channel'];
...
// Transforma o payload de string para objeto Json
payload = JSON.parse(payload)
response = null;

// has payload
var base64 = payload.params.payload;
if (base64)
    response = DecoderProIoT(base64ToByte(base64), 1);
